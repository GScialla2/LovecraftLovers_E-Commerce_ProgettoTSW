...
public class CartaDAO
{
    public static void aggiuntaCredenzialiPagamento(Carta p,String email)
    {
        ...
        try (Connection con = ConPool.getConnection()) {
            PreparedStatement ps = con.prepareStatement("SELECT numeroCarta FROM CartaDiCredito");
            ...
            if(f==0)
            {
                PreparedStatement ps1 = con.prepareStatement("INSERT INTO CartaDiCredito (numeroCarta,nomeIntestatario,dataScadenza,CVV,emailProprietario) VALUES (?,?,?,?,?)", Statement.RETURN_GENERATED_KEYS);
