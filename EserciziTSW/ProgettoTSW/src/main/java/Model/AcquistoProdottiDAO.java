...
public class AcquistoProdottiDAO
{
    ...
    public static ArrayList<AcquistoProdotti> doRetriveAcquistoUtente(String email)
    {
        ...
        try (Connection con = ConPool.getConnection())
        {
            PreparedStatement ps = con.prepareStatement("SELECT P.idProdotto,P.nomeProd,P.prezzo,A.quantitaAcquistata,C.Nome,C.Cognome,C.via,C.citta,C.cap,C.provincia FROM (Prodotto as P join Acquistare as A on P.idProdotto=A.idProdotto)join Cliente as C on C.email=A.emailCliente WHERE C.email=?");
            ps.setString(1, email);
            ResultSet rs = ps.executeQuery();
            while (rs.next())
            {
