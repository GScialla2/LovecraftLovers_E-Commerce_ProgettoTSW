...
public class UtenteDAO extends HttpServlet
{
    ...
    public static void doRegistrazione(Utente utente)
    {
        try (Connection con = ConPool.getConnection())
        {
            PreparedStatement ps = con.prepareStatement("INSERT INTO Cliente (email,passwordEmail,nome,cognome,dataDiNascita,numeroTelefono,codiceFiscale,via,citta,cap,provincia,nazione,amministratore) VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?)", Statement.RETURN_GENERATED_KEYS);
    ...
    public static void rendiAmministratore(String email)
    {
        try (Connection con = ConPool.getConnection())
        {
            PreparedStatement ps = con.prepareStatement("UPDATE Cliente SET amministratore=? WHERE email = ? ", Statement.RETURN_GENERATED_KEYS);
